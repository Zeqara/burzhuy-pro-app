...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(phoneForm) phoneForm.addEventListener('submit', (e) => {
        ...
        recaptchaVerifier.render().then((widgetId) => {
            ...
            console.error("Ошибка отрисовки reCAPTCHA:", err);
            showModal('Ошибка', 'Не удалось запустить проверку reCAPTCHA. Попробуйте обновить страницу.');
            sendCodeBtn.disabled = false;
            sendCodeBtn.textContent = 'Получить код';
        });
    });
    ...
    async function renderHistory() {
        ...
        snapshot.forEach(doc => {
            ...
            const date = report.checkDate && report.checkDate.toDate ? report.checkDate.toDate().toLocaleDateString('ru-RU') : 'без даты';
            const statusText = { pending: 'в ожидании', approved: 'принят', rejected: 'отклонен', paid: 'оплачен' }[report.status] || report.status;
            let commentHTML = '';
            if (report.status === 'rejected' && report.rejectionComment) {
