...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const cityListContainer = document.getElementById('city-list'),
        ...
        locationsHeader = document.getElementById('locations-header');
    ...
    if (registerForm) {
        registerForm.addEventListener('submit', e => {
            e.preventDefault();
            const n = registerNameInput.value, m = registerEmailInput.value, p = registerPasswordInput.value, t = registerPhoneInput.value;
            if (!n || !m || !p || !t) return alert('Заполните все поля!');
            auth.createUserWithEmailAndPassword(m, p)
                .then(c => db.collection('users').doc(c.user.uid).set({ name: n, phone: t, email: m, role: 'guest' }))
                .then(() => { alert('Успешно!'); registerForm.reset(); showLoginLink.click(); })
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', e => {
            e.preventDefault();
            const m = loginEmailInput.value, p = loginPasswordInput.value;
            if (!m || !p) return alert('Введите email и пароль.');
            auth.signInWithEmailAndPassword(m, p).catch(err => {
    ...
    if (checklistForm) {
        checklistForm.addEventListener('submit', e => {
            ...
            db.collection('reports').add(reportData).then(() => { alert('Спасибо за ваш отчёт ✅'); showScreen('main-menu-screen'); }).catch(error => { console.error("Ошибка: ", error); alert('Не удалось отправить отчет.'); });
        });
    }
