    ...
    function renderHistory() {
        ...
        db.collection('reports').where('userId', '==', user.uid).orderBy('checkDate', 'desc').get()
            .then(snapshot => {
                ...
                if (snapshot.empty) { historyListContainer.innerHTML = '<p>Вы еще не отправили ни одного отчета.</p>'; return; }
                snapshot.forEach(doc => {
                    ...
                    li.innerHTML = `<strong>${report.pointAddress}</strong><small>Дата: ${date} - Статус: ${statusText}</small>`;
