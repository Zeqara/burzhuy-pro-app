// =================================================================
// КОНФИГУРАЦИ-Я И ИНИЦИАЛИЗАЦИЯ FIREBASE
// =================================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function renderHistory() {
        ...
        snapshot.forEach(doc => {
            ...
            historyHTML += `<li class="menu-list-item history-item"><div class="status-indicator ${report.status}"></div><div><strong>${report.locationName}</strong><small>Дата: ${date} - Статус: ${statusText}</small></div></li>`;
        });
    ...
    menuButtons.forEach(b => b.addEventListener('click', (e) => {
        ...
        if (target === 'cooperation-screen') renderAvailableSchedules();
        else if (target === 'history-screen') { renderHistory(); showScreen(target); }
